...
public class Game implements Runnable, KeyListener {
	...
	private final int PAUSE = 80, // p key
			...
			MUTE = 77, // m-key mute
			//CLEAR = 67
			SPECIAL = 70, // F Key; StackOverFlow Nuke
			HYPER = 72;  // H Key; Tele-port to New Universe
	...
	private static final int SPAWN_NEW_SHIP_FLOATER = 40;
		...
		for (Movable movFriend : CommandCenter.getInstance().getMovFriends()) {
			for (Movable movFoe : CommandCenter.getInstance().getMovFoes()) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
					...
					// Code here Will remove objects that the stack overflow nuke touches
					// Stack Overflow nuke objects remain in the game despite collision
					else if (!(movFriend instanceof StackOverflow)){
						CommandCenter.getInstance().getOpsList().enqueue(movFriend, CollisionOp.Operation.REMOVE);
					}//end else
		...
		if (CommandCenter.getInstance().getFalcon() != null){
			...
			for (Movable movFloater : CommandCenter.getInstance().getMovFloaters()) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)) {
					...
					int numF = CommandCenter.getInstance().getNumFalcons();
					if (numF < 3)
					{
						CommandCenter.getInstance().setNumFalcons(numF+1);
					}
	...
	private void killFoe(Movable movFoe) {
		...
		if (movFoe instanceof Asteroid){
			...
			// Debris
				...
				// Spawn 8 Debris Objects
				CommandCenter.getInstance().addAst(.25);
				for(int i = 0; i < 8; i++)
				{
		...
		// Increment Score by 10 when User breaks a bug into smaller bugs or into debris
		CommandCenter.getInstance().addScore(10);
	...
	private void spawnNewShipFloater() {
		...
		if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 3) == 0) {
			...
			// Spawn Ghosts in the Snpachat World
			if (CommandCenter.getInstance().getPlanet().equals("Snapchat"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new Ghost(), CollisionOp.Operation.ADD);
			}

			// Spawn Hearts in the IG world
			else if (CommandCenter.getInstance().getPlanet().equals("Instagram"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new Heart(), CollisionOp.Operation.ADD);
			}

			// Spawn Maginfying Glass in the Google World
			else if (CommandCenter.getInstance().getPlanet().equals("Google"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new Glass(), CollisionOp.Operation.ADD);
			}

			// Spawn Triangles in the Youtube World
			else if (CommandCenter.getInstance().getPlanet().equals("YouTube"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new Triangle(), CollisionOp.Operation.ADD);
			}

			// Spawn Thumbs Up when in the FB world
			else if (CommandCenter.getInstance().getPlanet().equals("Facebook"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new ThumbsUp(), CollisionOp.Operation.ADD);
			}

			// Spawn Tinder Flame in the Tinder World
			else if (CommandCenter.getInstance().getPlanet().equals("Tinder"))
			{
				CommandCenter.getInstance().getOpsList().enqueue(new Flame(), CollisionOp.Operation.ADD);
			}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (fal != null) {
			...
			// When the User hits H and travels to new universe
			case HYPER:
				gmpPanel.hyper(gmpPanel.getGraphics());
				break;
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			switch (nKey) {
			case FIRE:
				CommandCenter.getInstance().getOpsList().enqueue(new NullPointer(fal), CollisionOp.Operation.ADD);
				Sound.playSound("laser.wav");
				break;


			// When the User Uses the Stack Over Flow Nuke
			case SPECIAL:

				// Make Sure there are still nukes available to  use
				if(CommandCenter.getInstance().getNuke() != 0)
				{
					// Spawn Nukes in all directions
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 0), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 45), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 90), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 135), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 180), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 225), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 270), CollisionOp.Operation.ADD);
					CommandCenter.getInstance().getOpsList().enqueue(new StackOverflow(fal, 315), CollisionOp.Operation.ADD);
					//Sound.playSound("laser.wav");
					CommandCenter.getInstance().subtractNuke();
			...
			//case CLEAR:
				//CommandCenter.getInstance().clearFoe();
				//break;
