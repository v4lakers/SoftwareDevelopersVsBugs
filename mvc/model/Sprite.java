...
public abstract class Sprite implements Movable {
	...
	protected double[] convertToPolarDegs(ArrayList<Point2D> pntPoints) {

	   double[] dDegs = new double[pntPoints.size()];

		int nC = 0;
		for (Point2D pnt : pntPoints) {
			dDegs[nC++]=(Math.toDegrees(Math.atan2(pnt.getY(), pnt.getX()))) * Math.PI / 180 ;
		}
		return dDegs;
	}
	//utility function to convert to polar
	protected double[] convertToPolarLens(ArrayList<Point2D> pntPoints) {
		...
		double dL = 0;
		for (Point2D pnt : pntPoints)
			if (hypot(pnt.getX(), pnt.getY()) > dL)
				dL = hypot(pnt.getX(), pnt.getY());

		int nC = 0;
		for (Point2D pnt : pntPoints) {
			if (pnt.getX() == 0 && pnt.getY() > 0) {
				dLens[nC] = hypot(pnt.getX(), pnt.getY()) / dL;
			} else if (pnt.getX() < 0 && pnt.getY() > 0) {
				dLens[nC] = hypot(pnt.getX(), pnt.getY()) / dL;
			} else {
				dLens[nC] = hypot(pnt.getX(), pnt.getY()) / dL;
			}
	...
	protected void assignPolarPoints(ArrayList<Point2D> pntCs) {
